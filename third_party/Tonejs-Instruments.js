...
var SampleLibrary = {
    ...
    setExt: function(newExt){
        ...
        this.ext = newExt;
        return console.log("sample extension set to "+ this.ext);
    },

    load: function(arg) {           //輸入ex: instruments: 'piano'
        var t, rt, i;
        arg ? (t = arg) : (t = {});
        t.instruments = t.instruments || this.list;         // 這邊this 是啥==?
        ...
        if(Array.isArray(t.instruments)) {
            for (i = 0; i <= t.instruments.length - 1; i++) {
                ...
                if(this.minify === true || t.minify === true) {
                    var minBy = 1;
                    if(Object.keys(newT).length >= 17) {                        //Object ?? keys?? 
                        minBy = 2;
                    }
